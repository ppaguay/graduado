...
public class FHistoria_laboral{
 ...
 boolean band= false;
 String sql = "insert into historia_laboral values (?,?,?,?,?,?,?)";
...
lstpar.add(new Parametro(5,obj.getEmpresa().getCodigo()));
...
throw ex;}
 ...
 boolean band= false;
 String sql = "update historia_laboral set codigo=?,cod_graduado=?,cod_tipocausasalida=?,cod_tipocargo=?,cod_empresa=?,fechainicio=?,fechafin=? where codigo=?  ";
ArrayList<Parametro> lstpar= new ArrayList<Parametro>();
...
try{ 
 String sql = "select codigo,cod_graduado,cod_tipocausasalida,cod_tipocargo,cod_empresa,fechainicio,fechafin from historia_laboral where   codigo=? ";
ArrayList<Parametro> lstpar = new ArrayList<Parametro>();
...
try{ 
 String sql = "select codigo,cod_graduado,cod_tipocausasalida,cod_tipocargo,cod_empresa,fechainicio,fechafin from historia_laboral; ";
...
obj.setEmpresa(FEmpresa.obtener(cr.getInt(5)));
